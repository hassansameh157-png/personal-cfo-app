...
const UI = {
  ...
  // ---- generic actions ---------------------------------------------------
  setPage(p) { this.app.state.page = p; this.app.state.moreOpen = false; this.app.state.quickAddOpen = false; this.app.state.txVisible = 25; this.render(); window.scrollTo({ top: 0 }); },
  ...
  catBar(name, v, max, kind, over, suffix) {
    ...
    return '<button class="bar-row" onclick="UI.viewCategoryTx(\'' + escJsArg(name) + "','" + escJsArg(kind) + '\')"><span class="bar-name"><span class="bar-name-ico">' + svgIcon(categoryIcon(name), 14) + '</span><span class="bar-name-text">' + esc(name) + "</span></span>" +
