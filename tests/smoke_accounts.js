...
(async () => {
  ...
  console.log("=== 1) Existing seed accounts (with transactions) have NO Delete action ===");
  await page.click(".navbtn:has-text('Accounts')"); await page.waitForTimeout(200);
  // Edit/Delete/Move all live behind the row/tile's "..." trigger now (see
  // UI.renderAcctActionSheet()), not always-visible inline buttons -- every
  // seed account renders as a .credit-card-tile (the seed data has no
  // "other"-type account at all, so .card-row never matches here); open
  // the first real seed tile's sheet (it has transactions against it in
  // the seed data, so isn't deletable) and check the sheet itself.
  const seedTiles = await page.locator(".credit-card-tile").count();
  await page.locator(".credit-card-tile").first().locator(".acct-more-btn").click();
  await page.waitForTimeout(150);
  console.log("seed tile count:", seedTiles, "| Delete action correctly absent from its sheet (has transactions):", await page.locator(".sheet-action:has-text('Delete')").count() === 0);
  await page.click(".sheet-backdrop", { force: true, position: { x: 5, y: 5 } }); await page.waitForTimeout(150);

  console.log("\n=== 2) New account with zero transactions has a Delete action ===");
  ...
  await freshRow.locator(".acct-more-btn").click(); await page.waitForTimeout(150);
  console.log("Delete action present on zero-tx account:", await page.locator(".sheet-action:has-text('Delete')").count() > 0);
  await page.click(".sheet-backdrop", { force: true, position: { x: 5, y: 5 } }); await page.waitForTimeout(150);

  console.log("\n=== 3) After a transaction against it, Delete action disappears ===");
  ...
  await page.click(".navbtn:has-text('Accounts')"); await page.waitForTimeout(200);
  await freshRow.locator(".acct-more-btn").click(); await page.waitForTimeout(150);
  console.log("Delete action gone after one transaction:", await page.locator(".sheet-action:has-text('Delete')").count() === 0);
  ...
  await cardRow.locator(".acct-more-btn").click(); await page.waitForTimeout(150);
  await page.click(".sheet-action:has-text('Edit')"); await page.waitForTimeout(200);
