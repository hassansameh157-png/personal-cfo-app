...
class Engine {
  ...
  nextOccurrence(r, from) {
    ...
    // Real bug fixed (#30): this used to read x.setMonth(new
    // Date(base).getMonth()) -- base's OWN month, at whatever moment this
    // happens to be called -- instead of a real month stored on the rule
    // itself, so a yearly rule's computed "next occurrence" silently
    // drifted to match whichever month you happened to check Forecast/
    // Dashboard in, rather than staying pinned to a real anniversary
    // month. r.month (0-11, set via FORMS()' own Month field) fixes that;
    // falls back to base.getMonth() only for a rule saved before this
    // field existed, so an untouched legacy rule keeps behaving exactly
    // as it already did rather than jumping to a new month on its own.
    if (r.freq === "yearly") { const month = r.month != null ? r.month : base.getMonth(); const x = new Date(base.getFullYear(), month, r.day || 1); if (x < base) x.setFullYear(x.getFullYear() + 1); return x; }
  ...
  FORMS() {
    ...
    // Real bug fixed (#30): a "yearly" recurring rule stored only a day of
    // month, never a month -- nextOccurrence()'s own yearly branch faked
    // one by reading whatever month `from` (usually "right now") happened
    // to be at call time, so the computed next occurrence silently drifted
    // to the CURRENT month instead of a real, fixed annual month. Locale-
    // aware month names (toLocaleDateString, same convention Cash Flow's
    // own month label already uses) rather than a hand-typed Arabic array
    // that could drift from the app's real date formatting elsewhere.
    const monthOptions = Array.from({ length: 12 }, (_, m) => ({ v: String(m), l: new Date(2000, m, 1).toLocaleDateString(this.state.lang === "ar" ? "ar-EG" : "en-GB", { month: "long" }) }));
    ...
    const cardStyleFields = () => [
      // One-tap curated looks (real gap fixed, iterated twice on direct user
      // feedback: round 1's Color/Secondary color/Pattern trio below was
      // flexible but never LOOKED like anything until composed by hand;
      // round 1's own presets -- soft blended gradients -- still read as
      // "generic gradient generator", not distinctive). This round's two
      // groups (GEM_THEMES/PLAIN_THEMES, ui.js) are deliberately opposite:
      // a faceted "gem cut" look for a genuinely standout card, and flat
      // "solid" for the explicit opposite ask, a plain one -- both one tap
      // via UI.setCardTheme, which only ever touches color/pattern (both
      // are one-color patterns, see cardBackground()). The trio below stays
      // untouched underneath for anyone fine-tuning past a preset, or
      // picking a color no preset uses -- "gem"/"solid" are real options in
      // the plain Pattern dropdown too, not preset-only. This field only
      // reserves the slot; the renderer pulls its own swatch data the same
      // "visual buttons drive the real controls" way the icon/color
      // swatches already do.
      ...
      D("pattern", this.L("Pattern", "النمط"), "select", { options: [
        ...
        { v: "gem", l: this.L("Faceted gem cut", "قطع مضلعة") },
        { v: "solid", l: this.L("Plain / solid", "سادة") }
      ] }),
  ...
  open(kind, pre) {
    ...
    const form = { date: this.today(), freq: "monthly", count: 12, down: 0, cost: 0, balloon: 0, type: kind === "recurring" ? "expense" : undefined, day: 1,
      ...
      // the actual renewal month the user is about to set up.
